...
overhead. This stage skips it:
  ...
  2. ASSEMBLE with ffmpeg DIRECTLY (no MoviePy frame loop): each still is a timed
     image input; consecutive stills are joined with a short `xfade` crossfade;
     the per-beat narration is delayed onto the timeline and mixed; a looped music
     bed is sidechain-DUCKED under the narration. Encoded with h264_nvenc when
...
plus a short silent tail so lines don't jump-cut; the xfade overlaps that tail.
...
CROSSFADE = 0.35       # crossfade between consecutive stills
TAIL = LINE_GAP + CROSSFADE   # trailing silence per line (the crossfade overlaps it)
...
def _plan(beats: list[Panel], mapping: dict[int, dict]
    ...
    Each item holds the still's source frame paths + layout, its duration, and its
    START on the final timeline (consecutive items overlap by CROSSFADE). Returns
    (items, total_duration, line_summaries) - the summaries drive the per-line log.
    """
    items: list[dict] = []
    lines: list[dict] = []
    for b in beats:
        ...
        if mode == "sequence" and n > 1:
            # Split the line's time evenly across its frames; the silent TAIL rides
            # on the last sub-still and the audio plays only over the first.
            # The N sub-stills crossfade with each other, which shortens the line's
            # wall-clock span by (N-1)*CROSSFADE - that would eat into the next
            # line's narration and overlap it. Add that time back to the line's
            # budget so the inter-line silence stays exactly LINE_GAP regardless of
            # how many frames the line has.
...
def _build_filtergraph(items: list[dict], music_in: int | None,
                       total: float) -> tuple[str, str, str | None]:
    """Build the ffmpeg filter_complex for the video xfade chain + audio mix.
    Image inputs are 0..n-1; narration audio inputs follow; music (if any) is at
    index `music_in`. Returns (filter_complex, video_label, audio_label|None)."""
    n = len(items)
    parts: list[str] = []

    # --- video: normalize each still, then chain xfades ---
    for i in range(n):
...
def _build_cmd(ffmpeg: str, items: list[dict], music_path: Path | None,
               total: float, out: Path, codec: str) -> list[str]:
    ...
    fc, vlabel, alabel = _build_filtergraph(items, music_in, total)
...
def _assemble_ffmpeg(items: list[dict], music_path: Path | None,
                     total: float, out: Path) -> str:
    ...
    last = ""
    for codec in codecs:
        cmd = _build_cmd(ffmpeg, items, music_path, total, out, codec)
...
# --------------------------------------------------------------------------- #
def _assemble_moviepy(items: list[dict], music_path: Path | None,
                      total: float, out: Path) -> str:
...
def run(m: Manifest) -> Manifest:
    ...
    try:
        ...
        try:
            ...
            codec = _assemble_moviepy(items, music_path, total, out)
            path = "moviepy"
    finally:
